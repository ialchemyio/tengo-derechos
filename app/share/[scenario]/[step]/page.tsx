...
export async function generateStaticParams() {
  ...
  for (const scenario of SHARE_SCENARIOS) {
    // Pre-generate up to 7 steps; getStepContent guards out-of-range at request time.
    for (let i = 1; i <= 7; i++) {
...
export async function generateMetadata({ params }: Props) {
  ...
  const content = getStepContent(scenario, Number(step), "en");
  if (!content) return { title: "Tengo Derechos" };
  const ogTitle = encodeURIComponent(content.command);
  const ogSub = encodeURIComponent(content.guide.title.en);
  const ogUrl = `${siteUrl}/api/og?title=${ogTitle}&subtitle=${ogSub}&step=${content.stepNumber}&scenario=${content.scenario}&locale=en`;
  const url = shareUrlFor(content.scenario, content.stepNumber, "en");
  return {
    title: `${content.command} · Tengo Derechos`,
    description: "Know your rights — step-by-step emergency guide.",
    alternates: {
      canonical: url,
      languages: {
    ...
    openGraph: {
      ...
      description: content.guide.title.en,
      url,
...
export default async function Page({ params }: Props) {
  ...
  const content = getStepContent(scenario, Number(step), "en");
  if (!content) notFound();
  return <SharePageView content={content} locale="en" />;
}
