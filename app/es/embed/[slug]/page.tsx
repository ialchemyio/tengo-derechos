...
export async function generateMetadata({ params }: Props) {
  ...
  const label = embedSlugLabel(slug as never, "es");
  return {
    title: `${label} · embed`,
    description: "Tarjeta de derechos integrable de Tengo Derechos.",
    robots: { index: false, follow: false },
    alternates: {
      canonical: `${siteUrl}/es/embed/${slug}`,
      languages: {
...
export default async function Page({ params }: Props) {
  const { slug } = await params;
  const data = await getEmbedContent(slug, "es");
  if (!data) notFound();
  return <EmbedCardView data={data} locale="es" />;
}
